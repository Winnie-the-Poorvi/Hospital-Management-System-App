...
public class My_Prescriptions extends AppCompatActivity {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot PresSS : dataSnapshot.getChildren()){
                    ...
                    if(id.equals(pidIntent)) {
                        Prescription p = new Prescription(medname,perDay,days,id,PresSS.getValue(Prescription.class).getPresid(),PresSS.getValue(Prescription.class).getPickedUp(),PresSS.getValue(Prescription.class).getRunsout(),PresSS.getValue(Prescription.class).getPudate());
                        pres.add(p);
                    }
                }
    ...
    private void createNotificationChannel()
    {
        if(Build.VERSION.SDK_INT>= Build.VERSION_CODES.O) {
            CharSequence name = "Procto Reminder Channel";
